  ...
  <div class="page-bottom-space">
    <div class="row bottom-space">
      ...
      <div class="col-12 col-md-8 text-right">
        <!-- <button class="btn btn-success btn-sm" v-on:click="showSmsModal" v-if="[1].indexOf(status) >= 0">{{$t('order.confirm')}}</button> -->
    ...
    <div class="bg-white rounded current-shadow px-3" v-if="order.courier && order.courier.idt_courier">
      <div class="row align-items-center">
        <div class="col-12 col-md-12 col-lg-5 my-3">
          <div class="row align-items-center">
            <div class="col-5 col-sm-4 col-md-5 col-lg-4">
              <img v-bind:src="order.courier.avatar_path" alt="" class="mw-100">
            </div>
            <div class="col-7 col-sm-8 col-md-7 col-lg-8 pl-0">
              <p class="mb-2"><b>{{$t('order.courier-data')}}:</b></p>
              <p class="mb-1">{{order.courier.name}}</p>
              <p class="mb-2" v-if="order.courier.phone"><a :href="'tel:+' + order.courier.phone" class="phone-link">{{order.courier.phone_format}}</a></p>
              <ul class="list-unstyled text-nowrap rating-stars mb-0">
                <li v-for="(item, index) in [1, 2, 3, 4, 5]" class="rating-stars__star d-inline-block"
                  :key="index"
                  v-bind:class="{'rating-stars__star--active' : (+(item - order.courier.rate) <= 0.3) && (+(item - order.courier.rate) <= 1.3),
                                'rating-stars__star--half' : (+(item - order.courier.rate) >= 0.3) && (+(item - order.courier.rate) <= 0.7)}"
                  >
              ...
              <span v-if="order.courier.rate" class="text-muted small">{{order.courier.rate.toFixed(1)}}</span>
              <button class="btn btn-default btn-sm d-none">Оставить отзыв</button>
            </div>
          </div>
        </div>
        <div class="col-12 col-md-12 col-lg-7 mt-3">
          <span v-if="status !== 7" class="mw-400 d-block">После того как заказ будет доставлен, вы сможете оценить курьера и оставить отзыв.</span>
          <form v-on:submit.prevent="sendFeedback" v-if="status === 7 && !order.rate.idt_rate">
          ...
          <div v-else>
            ...
            <p class="mb-2" v-if="order.rate && order.rate.feedback">«{{order.rate.feedback}}»</p>
            <ul class="list-unstyled text-nowrap rating-stars mb-3">
              <li v-for="(item, index) in order.rate.rate" class="rating-stars__star rating-stars__star--active d-inline-block" :key="index">
                <svg version="1.1" id="Слой_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                   width="24px" height="24px" viewBox="0 0 24 24" style="enable-background:new 0 0 24 24;" xml:space="preserve">
                  <path class="st0" d="M23.4,8.7L15.6,8l-3.1-7.6h0C12.5,0.1,12.2,0,12,0v19.7l0,0l6.7,4.2c0.5,0.3,1-0.1,0.9-0.7l-1.8-8l5.9-5.4
                    C24.2,9.4,24,8.7,23.4,8.7z" fill="#F6F6F6"/>
...
import GoBack from '../inner/GoBack'
import Thumbnail from '../utils/Thumbnail'
import ThumbnailsOuter from '../utils/ThumbnailsOuter'
import NewPassword from '../sign/NewPassword'
