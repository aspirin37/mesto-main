  ...
  <header class="fixed-top" v-on-click-outside="clickedOutside">
    <nav class="navbar navbar-expand-lg navbar-dark bg-theme">
      <div class="container">
        ...
        <!-- Logo -->
        <router-link to="/" class="navbar-brand mr-2 mr-xl-4" v-if="locale === 'ru'">
          <img src="@/assets/logo.svg" alt="logo" class="navbar-logo d-none d-xl-inline-block">
          <img src="@/assets/logo-mobile.svg" alt="logo" class="navbar-logo d-xl-none">
        </router-link>
        <router-link to="/" class="navbar-brand mr-2 mr-xl-4" v-else>
          <img src="@/assets/logo-en.svg" alt="logo" class="navbar-logo d-none d-xl-inline-block">
          <img src="@/assets/logo-mobile.svg" alt="logo" class="navbar-logo d-xl-none">
        </router-link>
        <!-- /Logo -->
        ...
        <!-- Menu icon -->
        <a href="#"
          class="nav-trigger d-lg-none"
          data-toggle="collapse"
          v-on:click.prevent="showNavbar = !showNavbar"
          v-bind:class="{'nav-trigger--opened' : showNavbar}"
        ><span></span></a>
        <!-- /Menu icon -->
        ...
        <div class="navbar-collapse pt-3 pt-sm-0" v-bind:class="{'collapse': !showNavbar}">
          ...
          <ul class="navbar-nav align-items-center">
            <li class="pr-1">
              <router-link tag="a" :to="{name: 'new-order'}" class="btn btn-light btn-sm">
                <img src="@/assets/icons/plus-sm-icon.svg" class="vertical-top btn-icon" alt="add">
                <span class="ml-1">Заказать курьера</span>
              </router-link>
            </li>  <!-- v-if="!($route.path === '/')" -->
            <router-link tag="li" to="/about" active-class="active rounded" class="nav-item relative text-center text-lg-left">
              <a href="#" class="nav-link px-3 text-nowrap">{{$t('navbar.about')}}</a>
            </router-link>
            <router-link tag="li" to="/for-business" active-class="active rounded" class="nav-item relative text-center text-lg-left">
              <a href="#" class="nav-link px-3 text-nowrap">{{$t('navbar.corporate')}}</a>
            </router-link>
            <router-link tag="li" to="/for-couriers" active-class="active rounded" class="nav-item relative text-center text-lg-left">
              <a href="#" class="nav-link px-3 text-nowrap">{{$t('navbar.couriers')}}</a>
            </router-link>
            <router-link tag="li" to="/faq" active-class="active rounded" class="nav-item relative text-center text-lg-left">
              <a href="#" class="nav-link px-3 text-nowrap">{{$t('navbar.faq')}}</a>
            </router-link>
            <router-link tag="li" to="/tariffs" active-class="active rounded" class="nav-item relative text-center text-lg-left">
              <a href="#" class="nav-link px-3 text-nowrap">{{$t('navbar.tariffs')}}</a>
            </router-link>
            ...
            <li class="nav-item text-center text-lg-left">
              <a href="#" class="nav-link top-nav__link d-md-none px-3" v-on:click.prevent="logout" v-if="isAuth">{{$t('signout')}}</a>
            </li>
          ...
          <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown d-none d-lg-block navbar-hover-menu" v-if="isAuth">
              <a href="#" class="nav-link dropdown-toggle navbar-user-logo pr-0 d-flex align-items-center">
      ...
      <div class="locale-block" v-if="isLocaleShow">
        <div class="container text-center">
          <ul class="list-inline list-unstyled locale-block__list">
            <li>
              <a href="#" class="locale-block__item locale-block__item--ru" v-bind:class="{'locale-block__item--active' : locale === 'ru'}" v-on:click="changeLang('ru')">
                <span class="d-none d-md-block">Русский</span>
              </a>
            </li>
            <li>
              <a href="#" class="locale-block__item locale-block__item--en" v-bind:class="{'locale-block__item--active' : locale === 'en'}" v-on:click="changeLang('en')">
                <span class="d-none d-md-block">English</span>
              </a>
            </li>
            <li>
              <a href="#" class="locale-block__item locale-block__item--de" v-bind:class="{'locale-block__item--active' : locale === 'de'}" v-on:click="changeLang('de')">
                <span class="d-none d-md-block">German</span>
              </a>
            </li>
          </ul>
        </div>
...
export default {
  ...
  computed: {
    ...
    isAuth () {
      return (this.user.authenticated && this.profile) || false
    },
    locale () {
